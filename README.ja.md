...
5. **歴史クイズ（Quiz - 全60問）**: 4ジャンル（全ジャンル、日本首相、米大統領、徳川将軍）から選べる全60問。選択肢ネタバレ完全排除、解説カードでの出典明記、進捗バー、紙吹雪演出付き。
6. **ダイレクトアクセス & リンク共有（SPA Deep Linking）**:
   - `#/timeline`（歴史年表）、`#/compare`（指導者比較）、`#/rankings`（統計ランキング）、`#/quiz`（歴史クイズ）各画面へのダイレクトアクセス対応。
   - `#/leader/jp-pm-1` や `#/leader/us-pres-16` など、特定の指導者の詳細モーダルを直接開くリンクの共有・コピー機能（Web Share API & クリップボード対応）。
7. **日英バイリンガル & ライト/ダークテーマ**: ワンタップで日本語/英語切替、デフォルトライトテーマ（ダークモード切替可能）。
