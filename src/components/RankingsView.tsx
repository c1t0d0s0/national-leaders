...
export const RankingsView: React.FC<RankingsViewProps> = ({
  ...
  const renderValueDisplay = (item: UniqueLeaderRankingItem) => {
    switch (currentMetric) {
      case 'height':
        return (
          <div>
            ...
            {item.heightSource && (
              <span className="block text-[10px] text-slate-400 truncate max-w-[200px]" title={item.heightSource}>
                {item.heightSource}
              </span>
            )}
          </div>
      ...
      case 'weight':
        return (
          <div>
            ...
            {item.weightSource && (
              <span className="block text-[10px] text-slate-400 truncate max-w-[200px]" title={item.weightSource}>
                {item.weightSource}
              </span>
            )}
          </div>
        ...
        return (
          <div>
            ...
            <span className="block text-[11px] text-slate-500 font-medium">
              約 {(item.totalReignDays / 365.25).toFixed(1)} {t.leaderCard.years} {item.termCount > 1 ? `(${language === 'ja' ? `通算${item.termCount}期` : `${item.termCount} terms`})` : ''}
            </span>
          </div>
        );
      case 'age_young':
          ...
          {rankingList.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                {/* Avatar */}
                <LeaderAvatar leader={item.representativeLeader} language={language} size="sm" className="shrink-0" />
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white group-hover:text-amber-600 dark:group-hover:text-amber-400 transition truncate">
